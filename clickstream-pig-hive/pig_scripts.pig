logs = LOAD '/user/cloudera/pig_hive/Omniture.0.tsv' USING PigStorage('\t') 
AS 
...
logs1 = FOREACH logs GENERATE col_1 AS logdate, col_7 AS ip, col_12 AS url, col_13 AS swid, col_49 AS city, col_50 AS country, col_52 AS state;

logs2 = FOREACH logs1 GENERATE logdate, url, ip, city, UPPER(state) AS state, UPPER(country) AS country, swid;

store logs2 into '/user/cloudera/pig_hive/pig_out_parsed_logs';

products = LOAD '/user/cloudera/pig_hive/products.tsv' USING PigStorage('\t') AS (url:chararray,category:chararray);

store products into '/user/cloudera/pig_hive/pig_out_products';

users = LOAD '/user/cloudera/pig_hive/users.tsv' USING PigStorage('\t') AS (SWID:chararray,BIRTH_DT:chararray,GENDER_CD:chararray);

store users into '/user/cloudera/pig_hive/pig_out_users';
